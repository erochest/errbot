...
Zayin::Rake::docco :docco, Dir.glob(File.join('**', '*.hs'))

desc 'Creates the docs and commits onto the gh-pages branch.'
task :ghpages, [:msg] => [:docs] do |t, args|
  msg    = args[:msg] || 'Updated docs.'
  branch = `git branch | grep '*' | cut -f2 -d' '`
  tmp    = "/tmp/#{PROJECT}-docs"

  FileUtils.rmtree(tmp, :verbose => true) if File.exists?(tmp)
  FileUtils.mv("docs", tmp, :verbose => true)

  sh %{git stash}
  sh %{git checkout gh-pages}

  FileUtils.rmtree("docs", :verbose => true) if File.exists?("docs")
  FileUtils.mv(tmp, "docs", :verbose => true)
...
task :tobin => :build do
  FileUtils.cp("dist/build/#{PROJECT}/#{PROJECT}",
               File.expand_path("~/bin/#{PROJECT}"),
               :verbose => true)
end
...
namespace :release do
  ...
  task :strip do
    sh %{strip -p --strip-unneeded --remove-section=.comment -o ./#{PROJECT} ./dist/build/#{PROJECT}/#{PROJECT}}
  end

  desc 'This copies the file to the ~/bin directory.'
  task :tobin => ['release:build', 'release:strip'] do
    FileUtils.cp('#{PROJECT}', File.expand_path('~/bin/#{PROJECT}'), :verbose => true)
  end
end
